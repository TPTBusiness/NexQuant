...
class RDAgentLog(SingletonBaseClass):
    ...
    def _configure_console_sinks(cls) -> None:
        ...
        # Filter to downgrade litellm messages from warning to info
        LITELLM_MESSAGES = [
        ...
        def litellm_info_filter(record):
            msg = str(record.get("message", ""))
            # Check if this is a litellm info message
            if any(kw in msg for kw in LITELLM_MESSAGES):
                record["message"] = f"Info: {msg}"
                record["level"].name = "INFO"
                record["level"].no = 20  # INFO level
            return not raw_filter(record)

        if LOG_SETTINGS.format_console is not None:
            logger.add(sys.stdout, format=LOG_SETTINGS.format_console, filter=litellm_info_filter)
        else:
            logger.add(sys.stdout, filter=litellm_info_filter)
