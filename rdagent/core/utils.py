...
class SingletonBaseClass:
    ...
    def __reduce__(self) -> NoReturn:
        """
        NOTE:
        When loading an object from a pickle, the __new__ method does not receive the `kwargs`  # nosec
        ...
        """
        msg = f"Instances of {self.__class__.__name__} cannot be pickled"  # nosec
        raise pickle.PicklingError(msg)  # nosec
...
def _subprocess_wrapper(f: Callable, seed: int, args: list) -> Any:  # nosec
    """
    It is a function wrapper. To ensure the subprocess has a fixed start seed.  # nosec
    """
...
def multiprocessing_wrapper(func_calls: list[tuple[Callable, tuple]], n: int) -> list:
    ...
    n : int
        the number of subprocesses  # nosec
    ...
    with mp.Pool(processes=max(1, min(n, len(func_calls)))) as pool:
        results = [
            pool.apply_async(_subprocess_wrapper, args=(f, LLM_CACHE_SEED_GEN.get_next_seed(), args))  # nosec
            for f, args in func_calls
        ]
        return [result.get() for result in results]


def cache_with_pickle(hash_func: Callable, post_process_func: Callable | None = None, force: bool = False) -> Callable:  # nosec
    """
    This decorator will cache the return value of the function with pickle.  # nosec
    ...
    specified by RD_AGENT_SETTINGS.pickle_cache_folder_path_str with name hash_key.pkl.  # nosec
    ...
    force : bool, optional
        If True, the cache will be used even if RD_AGENT_SETTINGS.cache_with_pickle is False, by default False.  # nosec
    """

    def cache_decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def cache_wrapper(*args: Any, **kwargs: Any) -> Any:
            if not RD_AGENT_SETTINGS.cache_with_pickle and not force:  # nosec
                return func(*args, **kwargs)

            target_folder = Path(RD_AGENT_SETTINGS.pickle_cache_folder_path_str) / f"{func.__module__}.{func.__name__}"  # nosec
            ...
            if cache_file.exists():
                with cache_file.open("rb") as f:
                    cached_res = pickle.load(f)  # nosec
